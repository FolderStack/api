...
const fetchItemsRecursively = (
    command: DynamoCommand,
    paginate = false
): TE.TaskEither<Error, DynamoCommandOutput> => pipe(
    TE.tryCatch(
    ...
    TE.chain(response => {
        ...
        const nextCommand = command instanceof QueryCommand ?
            new QueryCommand({
                ...command.input,
        ...
        return pipe(
            ...
            TE.map(nextResponse => {
                const count = (response.Count ?? 0) + (nextResponse.Count ?? 0);
                const scannedCount = (response.ScannedCount ?? 0) + (nextResponse.ScannedCount ?? 0);
                return {
                    ...response,
